...
\node[core, right=23mm of content] (sui) {Sui protocol state\\objects and events};
\node[box, right=33mm of sui] (sdk) {SDKs, indexers,\\frontends, agents};
\node[box, below=20mm of sui] (manifestNode) {Deployment manifest\\package and object bindings};
...
\breakcode{ArtifactSeries} & Stable identity of a continuing artifact. Stores type, code, controller binding and compatibility owner mirror, version IDs, current version, official comments tree, official likes book, status, UI status, metadata extensions, and timestamps. \\
\breakcode{ControllerNFT} & Transferable control asset for one artifact series. Stores series binding, artifact type, control-record binding, issuance metadata, and transfer-relevant state. Holding the NFT is the canonical series-control authority. \\
\breakcode{ArtifactControlRecord} & Canonical control-binding object. Stores the series ID, controller NFT ID, current controller mirror, compatibility owner mirror, official comments tree binding, authority mode, and timestamps used by contracts, SDKs, and indexers to resolve series authority. \\
...
\breakcode{CommentsTree} & Official comment space for a series. Stores target series, target artifact type, root comment, nodes table, controller-aligned moderation owner mirror, status, fee manager binding, max inline size, depth limit, and likes book ID. \\
...
Every controlled series MUST also have exactly one canonical controller asset
represented by a \code{ControllerNFT} and exactly one \code{ArtifactControlRecord}
that binds the series, the controller NFT, and the official comments tree. The
controller NFT is the source of write authority for series-scoped operations.
The control record exists so contracts, SDKs, and indexers can resolve control
state deterministically without inferring it only from frontend conventions.

Control authority is distinct from content license and historical authorship.
Transferring the controller NFT changes who may publish new versions, update
series-level metadata, and exercise artifact-scoped moderation authority. It
does not rewrite prior versions, change the recorded author of a version, or
replace the license terms committed by earlier version records.
...
\begin{enumerate}[leftmargin=*]
  ...
  \item mint the series \code{ControllerNFT} and create the \code{ArtifactControlRecord};
  \item create the official comments tree and likes book;
  \item bind controller authority to the official comments tree moderation boundary;
  \item share the series, comments tree, likes book, and typed version record, and return the controller NFT to the publisher wallet;
...
An add-version transaction MUST be authorized by the holder of the series
\code{ControllerNFT} or by a protocol-recognized execution path that presents
controller authority for the same series. It MUST match the series artifact
type, MUST keep the series within the maximum version count, MUST use official
root/registry/vault/fee manager bindings, and MUST produce a typed version
record for the same artifact type. It emits \code{ArtifactVersionAddedEvent}.

\subsection{Ownership Transfer}

Artifact control is transferred by transferring the series \code{ControllerNFT}.
The receiving address becomes the canonical controller and therefore gains the
right to add versions, update series metadata, and administer artifact-scoped
moderation policy. Implementations MAY maintain series-level and comments-tree
owner mirrors for compatibility and display, but such mirrors are not the
canonical source of control authority.

Controller transfer MUST NOT mutate prior version records, prior version
authors, committed content hashes, historical Walrus references, or prior
license selections. It changes future operational authority over the series; it
does not rewrite the historical evidence already committed by earlier versions.

Indexers and applications SHOULD resolve current series control from the
\code{ControllerNFT} plus the bound \code{ArtifactControlRecord}, not from stale
display mirrors or cached frontend ownership assumptions. If an application
shows a compatibility owner mirror, that mirror SHOULD be treated as advisory
display state rather than as the final source of protocol authority.

Marketplace listing, escrow, auction, lending, or bundled sale of a
\code{ControllerNFT} MAY be implemented by external Sui infrastructure. Such
commercial flows are outside PaperProof's protocol scope until a valid NFT
transfer settles on chain. Once settlement occurs, the receiving holder becomes
the canonical controller under the same artifact series ID and control-record
binding.
...
For controller-aware views, the indexer SHOULD resolve and expose the current
controller NFT owner, controller NFT ID, control-record ID, and any relevant
compatibility mirrors separately. A client MUST NOT infer canonical artifact
control from a stale mirrored owner field when controller-object state is
available.
...
\begin{itemize}[leftmargin=*]
  ...
  \item expose controller-resolution, controller-transfer, and authority-check helpers that distinguish control rights from content license fields and historical authorship;
